...
//
// Laeuft via tsx (npm run verify:sandbox). Zieldatei ist sandbox-canary.txt,
// eine Datei ohne schuetzenswerten Inhalt, die in .claude/settings.json unter
// deny steht. Echtdaten werden nie angefasst.
//
...
try {
  ...
  if (code === 'EACCES' || code === 'EPERM') {
    console.log(`Sandbox aktiv: Lesezugriff auf sandbox-canary.txt scheitert mit ${code}.`)
    console.log('Die Deny-Regeln werden auf Dateisystemebene durchgesetzt, nicht nur im Werkzeug.')
    process.exit(0)
  }
  console.error(`Unerwarteter Fehler beim Lesen: ${(err as Error).message}`)
  process.exit(1)
}
